      ...
      <div class="pager-row-column-cell" v-for="(item, index) in data" :key="index">
        <div class="erek-paper-container-1">
          <div class="erek-paper-content-2">
            <img
              class="erek-paper-avatar-3"
              src="https://gw.alipayobjects.com/zos/rmsportal/nxkuOJlFJuAUhzlMTCEe.png"
              alt
            >
            <h4 class="erek-pager-text-4">{{ item.username }}</h4>
            <h4 class="erek-pager-text-4">级别 : {{ item.grade }}</h4>
            <h4 class="erek-pager-text-4">职位 : {{ item.job }}</h4>
          </div>
          <div class="erek-paper-action-5">
            <button
              class="erek-button-base-6 update-button"
              @click="onHandleActions('view', item)"
            >查看</button>
            <button
              class="erek-button-base-6 update-button"
              @click="onHandleActions('update', item)"
            >编辑</button>
            <button
              class="erek-button-base-6 delete-button"
              @click="onHandleActions('delete', item)"
            >删除</button>
          </div>
        </div>
      </div>
