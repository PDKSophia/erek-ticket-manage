  ...
  <div>
    ...
    >
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="onHandleClickSubmit('bus_pos_form')">确定</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    clearData() {
      Object.keys(this.bus_pos_form).forEach(key => {
        this.bus_pos_form[key] = ''
      })
    },
    onHandleClickSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let emitOptions = { ...this.bus_pos_form }
    ...
    formData: {
      handler(newVal) {
        if (newVal === null) {
          ...
          this.$nextTick(() => {
            this.bus_pos_form = { ...this.bus_pos_form, ...newVal }
          })
        }
      },
      deep: true
