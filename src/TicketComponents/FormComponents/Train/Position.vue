  ...
  <div>
    ...
    >
      ...
      <div slot="footer">
        ...
        <Button type="primary" @click="onHandleClickSubmit('train_pos_form')">确定</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    clearData() {
      Object.keys(this.train_pos_form).forEach(key => {
        this.train_pos_form[key] = ''
      })
    },
    onHandleClickSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let emitOptions = { ...this.train_pos_form }
    ...
    formData: {
      handler(newVal) {
        if (newVal === null) {
          ...
          this.$nextTick(() => {
            this.train_pos_form = { ...this.train_pos_form, ...newVal }
          })
        }
      },
      deep: true
